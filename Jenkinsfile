   ...
   stages {
       ...
       stage('Deploy') {
            steps {
                ...
                echo 'Deploying....'
                sh 'cf push account --random-route -p applications/account/build/libs/applications/account-0.0.1-SNAPSHOT.jar'
                sh 'cf push order --random-route -p applications/order/build/libs/applications/order-0.0.1-SNAPSHOT.jar'
                sh 'cf push product --random-route -p applications/product/build/libs/applications/product-0.0.1-SNAPSHOT.jar'
                sh 'cf push shipment --random-route -p applications/shipment/build/libs/applications/shipment-0.0.1-SNAPSHOT.jar'
            }
        }
   }
}
